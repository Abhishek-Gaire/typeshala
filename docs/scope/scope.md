...
### 12. Polish and packaging three systems · done
...
- [x] Traditional drill pattern per spec 0015 (retirement note added to spec 0012)
...
### 15. Nepali Traditional drill content pattern · done

Add mirrored finger pair drills for Traditional layout on Home, Top, Bottom, All screens with correct Preeti matra combos, matching the English pattern from spec 0013.
**Done when:** you can switch to Traditional, pick Home/Top/Bottom/All at each level, and type drills with real finger pairs and matra combos.

- [x] Design it (spec): `/architect Nepali Traditional drill content pattern`
- [x] Build it: `/develop Nepali Traditional drill content pattern`
  - [x] Add 12 Traditional rows with mirrored pairs and matra combos (AC-1, AC-2, AC-3, AC-4)
  - [x] L2 same-hand and L3 mixed-hand groups with no repeats (AC-5, AC-6)
  - [x] Lint passes on all rebuilt rows (AC-9)
- [x] Verify it: `/check verify Nepali Traditional drill content pattern`
- [x] Test it: `/test Nepali Traditional drill content pattern`
      Spec [0015](../specs/0015-nepali-traditional-drill-pattern.md) · code in `src/domain/classicDrills.ts`
...
- [x] Design it (spec): `/architect android immersive input fixes`
- [ ] Build it: `/develop android immersive input fixes`
  - [x] Native immersive shell: manifest lock plus sticky bars plus core pin (AC-2, AC-3, AC-4)
  - [x] Board tap path plus device keyboard suppressed (AC-1)
